...
rule extract_tags:
    ...
    shell: """
    singularity exec -B {FASTQ_FOLDER} -B {WORKING_FOLDER} -B {NORM_FOLDER} -B {CODE_HOME} \
    --env "PATH=/home/ubuntu/miniforge3/bin:${{PATH}}" nanoseq_1.1.0.sif /bin/bash -c "python {CODE_HOME}/extract_tags.py -a {input[0]} -b {input[1]} -c {output[0]} -d {output[1]} -m 3 -s 2 -l 150 1> {log[0]} 2> {log[1]}"
"""
...
rule add_rc_mc_tags:
    ...
    --env "PATH=/home/ubuntu/miniforge3/bin:${{PATH}}" nanoseq_1.1.0.sif /bin/bash -c "bamsormadup tmpfile={WORKING_FOLDER}/tmp_{wildcards.sample}_rcmc inputformat=sam rcsupport=1 threads=8 < {input} > {output}"
...
rule nanoseq_cov:
    ...
    singularity exec -B {FASTQ_FOLDER} -B {WORKING_FOLDER} -B {NORM_FOLDER} -B {CODE_HOME} -B {BWA_FOLDER} \
...
rule nanoseq_partition:
    ...
    shell: """
    singularity exec -B {FASTQ_FOLDER} -B {WORKING_FOLDER} -B {NORM_FOLDER} -B {CODE_HOME} -B {BWA_FOLDER} \
    --env "PATH=/home/ubuntu/miniforge3/bin:/opt/wtsi-cgp/bin:${{PATH}}" nanoseq_1.1.0.sif /bin/bash -c "python {CODE_HOME}/python/runNanoSeq.py --out {WORKING_FOLDER}/{wildcards.sample} -t 1 -A {input[0]} -B {input[1]} -R {BWA_INDEX} part -n 8 2>{log}"
    """
